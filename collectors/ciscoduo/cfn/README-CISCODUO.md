# ciscoduo-collector.template

Alert Logic Cisco Duo Log Collector CloudFormation template.

 
# Before you begin

This procedure requires administrative permissions in AWS and your Alert Logic 
Cloud Insight account. You also need to download the Alert Logic custom [CFT](https://github.com/alertlogic/paws-collector/blob/master/collectors/ciscoduo/cfn/ciscoduo-collector.template).
...
`https://s3.amazonaws.com/alertlogic-collectors-us-east-1/cfn/ciscoduo-collector.template`
1. In the next step, you will need:
   ...
   - `AlApplicationId` - use `Ciscoduo` (Alert Logic Application Id for collector logs)  
   ...
   - `CiscoduoObjectNames` - Define Object names. Please pass JSON formatted list. Possible values are ["Authentication","Administrator","Telephony","OfflineEnrollment"]
   - `CiscoduoClientId` - Cisco Duo Integration key.
   - `CiscoduoEndpoint` - Cisco Duo API hostname.
   - `CiscoduoSecret` - Cisco Duo Secret key
