# auth0-collector.template

Alert Logic Auth0 Log Collector CloudFormation template.

 
# Before you begin

This procedure requires administrative permissions in AWS and your Alert Logic 
Cloud Insight account. You also need to download the Alert Logic custom [CFT](https://github.com/alertlogic/paws-collector/blob/master/collectors/auth0/cfn/auth0-collector.template).
...
`https://s3.amazonaws.com/alertlogic-collectors-us-east-1/cfn/auth0-collector.template`
1. In the next step, you will need:
   ...
   - `AlApplicationId` - use `auth0` (Alert Logic Application Id for collector logs)  
