# salesforce-collector.template

Alert Logic Salesforce Log Collector CloudFormation template.

 
# Before you begin

This procedure requires administrative permissions in AWS and your Alert Logic 
Cloud Insight account. You also need to download the Alert Logic custom [CFT](https://github.com/alertlogic/paws-collector/blob/master/collectors/salesforce/cfn/salesforce-collector.template).
...
`https://s3.amazonaws.com/alertlogic-collectors-us-east-1/cfn/salesforce-collector.template`
1. In the next step, you will need:
   ...
   - `AlApplicationId` - use `salesforce` (Alert Logic Application Id for collector logs)  
   ...
   - `PawsCollectorTypeName` - use `salesforce`
   - `SalesforceClientId` - Salesforce Client ID 
   - `SalesforceEndpoint` - Salesforce Endpoint
   - `SalesforceObjectNames` - Define Object name, Please pass JSON formatted list. Possible values are ["LoginHistory", "EventLogFile","ApiEvent", "LoginEvent","LogoutEvent", "LoginAsEvent"]
