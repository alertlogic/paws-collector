# ciscoamp-collector.template

Alert Logic Cisco AMP Log Collector CloudFormation template.

 
# Before you begin

This procedure requires administrative permissions in AWS and your Alert Logic 
Cloud Insight account. You also need to download the Alert Logic custom [CFT](https://github.com/alertlogic/paws-collector/blob/master/collectors/ciscoamp/cfn/ciscoamp-collector.template).
...
`https://s3.amazonaws.com/alertlogic-collectors-us-east-1/cfn/ciscoamp-collector.template`
1. In the next step, you will need:
   ...
   - `AlApplicationId` - use `Ciscoamp` (Alert Logic Application Id for collector logs)  
   ...
   - `CiscoampClientId` - Cisco AMP client ID
   - `CiscoampEndpoint` - Cisco AMP endpoint
   - `CiscoampResourceNames` - Define Resource name, Please pass JSON formatted list. Possible values are ["AuditLogs","Events"]
   - `CiscoampSecret` - Cisco AMP client API Key
   ...
   - `PawsCollectorTypeName` - `ciscoamp`
